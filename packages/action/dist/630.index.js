...
function pkgForSomeOtherPlatform() {
  const libMainJS = /*require.resolve*/(84630);
...
function downloadedBinPath(pkg, subpath) {
  const esbuildLibDir = path.dirname(/*require.resolve*/(84630));
...
function generateBinPath() {
  ...
  let binPath;
  try {
    binPath = __WEBPACK_EXTERNAL_createRequire(import.meta.url).resolve(`${pkg}/${subpath}`);
  } catch (e) {
    binPath = downloadedBinPath(pkg, subpath);
    if (!fs.existsSync(binPath)) {
      try {
        __WEBPACK_EXTERNAL_createRequire(import.meta.url).resolve(pkg);
      } catch {
...
for your current platform.`);
  ...
  if (/\.zip\//.test(binPath)) {
    let pnpapi;
    try {
      pnpapi = __webpack_require__(15301);
    } catch (e) {
    }
    if (pnpapi) {
...
var path2 = __webpack_require__(16928);
var fs2 = __webpack_require__(79896);
var os2 = __webpack_require__(70857);
var tty = __webpack_require__(52018);
var worker_threads;
if (process.env.ESBUILD_WORKER_THREADS !== "0") {
  try {
    worker_threads = __webpack_require__(28167);
  } catch {
  }
